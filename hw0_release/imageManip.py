...
def rgb_exclusion(image, channel):
    ...
    ### YOUR CODE HERE
    dict = {'R': [0, 1, 1], 'G': [1, 0, 1], 'B': [0, 0, 1]}
    out = image * dict[channel]
    ### END YOUR CODE
...
def mix_images(image1, image2, channel1, channel2):
    ...
    out = None
    ### YOUR CODE HERE
    image1 = image1[:, 0:int(image1.shape[1] / 2), :]
    image2 = image2[:, 0:int(image2.shape[1] / 2), :]
...
def mix_quadrants(image):
    ...
    tr = image[a:2*a,0:b,:]
    bl = image[0:a,b:2*b,:]
    br = image[a:2 * a, b:2 * b, :]
    tl = rgb_exclusion(tl,'R')
    tr = dim_image(tr)
    bl = 0.5*bl
    br = rgb_exclusion(br,'R')
    out1 = np.concatenate((tl,tr),axis=0)
    out2 = np.concatenate((bl,br),axis=0)
